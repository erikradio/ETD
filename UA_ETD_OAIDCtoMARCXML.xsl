...
<!-- stylesheet transform OAI-DC Theses records to MARCXML for UA Libraries -->
<xsl:stylesheet version="1.0" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/1.1"
...
http://www.openarchives.org/OAI/2.0/oai_dc.xsd" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="OAI-PMH">
		...
		<xsl:for-each select="//ListRecords/record">
			<xsl:variable name="scontrol" select = "header/identifier/." />
			<!--<xsl:for-each select="metadata/oai_dc:dc">
				<xsl:call-template name="oai_dc:dc">
					<xsl:with-param name="scontrol" select="$scontrol" />
				</xsl:call-template>
			</xsl:for-each>-->
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match="oai_dc:dc">
		<!--<xsl-param name="scontrol" />-->
		<xsl:variable name="date1" select="normalize-space((dc:date[1]))" />
		<xsl:variable name="lang">
			<xsl:if test="dc:language = 'en_US'">eng</xsl:if>
			<xsl:if test="dc:language = 'en'">eng</xsl:if>
			<xsl:if test="dc:language = 'es'">spa</xsl:if>
			<xsl:if test="dc:language = 'fr'">fre</xsl:if>
					</xsl:variable>
		<!--<xsl:variable name="date2" select="normalize-space(translate(substring-after(dc:description[1], 'date: '),'.',' '))" />-->
		<record>
...
<subfield code="a">dc</subfield>
			...
			<!--<datafield tag="090" ind1=" " ind2=" ">
				<subfield code="a">LD4330 <xsl:value-of select="$date2" /></subfield>
			</datafield>-->
			...
			<xsl:if test="dc:title[1]">
				<xsl:call-template name="title_template">
					<xsl:with-param name="string" select="normalize-space(dc:title[1])" />
					<xsl:with-param name="field" select="'245'" />
					<xsl:with-param name="ind1" select="'1'" />
					<xsl:with-param name="ind2" select="' '" />
					<xsl:with-param name="subc" select="dc:creator[1]" />
				</xsl:call-template>
			</xsl:if>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<datafield tag="264" ind1=" " ind2="1">
						<subfield code="a">Tucson</subfield>
						<subfield code="b">University of Arizona</subfield>
					</datafield>
			...
			<xsl:for-each select="dc:type">
				<xsl:if test="contains(.,'Thesis')">
					<datafield tag="502" ind1=" " ind2=" ">
						<subfield code="a">Thesis -- University of Arizona, <xsl:value-of select="$date1" />.</subfield>
					</datafield>
				</xsl:if>
				<xsl:if test="contains(.,'Dissertation')">
					<datafield tag="502" ind1=" " ind2=" ">
						<subfield code="a">Dissertation (Ph.D) -- University of Arizona, <xsl:value-of select="$date1" />.</subfield>
					</datafield>
				</xsl:if>
			</xsl:for-each>
			<xsl:if test="dc:type"></xsl:if>
			...
			<xsl:if test="dc:description">
				<datafield tag="520" ind1="3" ind2=" ">
					<subfield code="a"><xsl:value-of select="normalize-space(dc:description)"/></subfield>
				</datafield>
			</xsl:if>
...
<xsl:value-of select="."/>
			...
			<!--<xsl:if test="dc:degree[1]">
				<datafield tag="690" ind1=" " ind2=" ">
					<subfield code="a">Theses, OSU</subfield>
					<subfield code="x"><xsl:value-of select="normalize-space(substring-after(dc:degree[1],'in '))" />.</subfield>
				</datafield>
			</xsl:if>-->
	...
	<xsl:template name="title_template">
		...
		<datafield>
			...
			<xsl:attribute name="ind2">
				<xsl:choose>
					<xsl:when test="starts-with($string,'The ') or 
starts-with($string,'A ') or 
starts-with($string,'An ') or 
starts-with($string,'And ')">
						<xsl:if test="starts-with($string, 'The ')">4</xsl:if>
						<xsl:if test="starts-with($string, 'A ')">2</xsl:if>
						<xsl:if test="starts-with($string, 'An ')">3</xsl:if>
						<xsl:if test="starts-with($string, 'And ')">4</xsl:if>
					</xsl:when>
					<xsl:otherwise>0</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
			<xsl:if test="$subc!=''">
				<subfield code="a"><xsl:value-of select="normalize-space(translate($string,'.',' '))" /> /</subfield>
				<!--<subfield code="c"><xsl:value-of select="substring-after($subc, ', ')" /><xsl:text></xsl:text><xsl:value-of select="substring-before($subc, ', ')" />.</subfield>-->
				<subfield code="c"><xsl:value-of select="dc:creator[1]" /></subfield>
			</xsl:if>
			<xsl:if test="$subc=''">
				<subfield code="a">
	...
	<xsl:template name="persname_template">
		...
		<datafield>
			...
			<xsl:if test="contains($string, '(')!=0">
				...
				<xsl:variable name="q_del" select="substring-after($string, '(')" />
				<xsl:if test="contains($q_del, ',')!=0">
					<subfield code="q">
						<xsl:text>(</xsl:text>
						<xsl:value-of select="substring-before($q_del, ',')" />
						<xsl:text>, </xsl:text>
					</subfield>
				</xsl:if>
				<xsl:if test="$q_del !='' and contains($q_del, ',')=0">
					<subfield code="q">
						<xsl:text>(</xsl:text>
						<xsl:value-of select="$q_del" />
					</subfield>
				</xsl:if>
				<xsl:if test="contains($string, '-')">
					<xsl:variable name="str1" select="translate(substring-before($string, '-'), ' ', '')" />
					<xsl:variable name="str2" select="translate(substring-after($string, '-'), ' ', '')" />
					<subfield code="d"><xsl:value-of select="substring($str1, string-length($str1)-3,4)" />-<xsl:value-of select="substring($str2, 1,4)" />.</subfield>
				</xsl:if>
			</xsl:if>
			<xsl:if test="contains($string, '(')=0">
				<xsl:variable name="tmp1" select="substring-before($string, '-')" />
				<xsl:variable name="tmp2" select="string-length($tmp1)-6" />
				<xsl:if test="contains($string, '-')!=0">
					<subfield code="a"><xsl:value-of select="substring($tmp1, 1, $tmp2)" />, </subfield>
				</xsl:if>
				<xsl:if test="contains($string, '-')=0">
					<subfield code="a">
						<xsl:value-of select="substring($tmp1, 1, $tmp2)" />
					</subfield>
				</xsl:if>
				<xsl:if test="contains($string, '-')">
		...
		<datafield>
			...
			<xsl:choose>
				<xsl:when test="contains($string, '. ')!=0">
					...
					<subfield code="a"><xsl:value-of select="$tmp1" />. </subfield>
	...
	<xsl:template name="corpname_tokenizeb">
		...
		<xsl:if test="contains($string, '. ')!=0">
			<xsl:variable name="str1" select="substring-before($string, '. ')" />
			<xsl:variable name="str2" select="substring-after($string, '. ')" />
			<subfield code="b"><xsl:value-of select="$str1" />. </subfield>
			<xsl:call-template name="corpname_tokenizeb">
			...
			<xsl:choose>
				<xsl:when test="contains($string, '--')!=0">
					...
					<xsl:call-template name="subject_tokenize">
						<xsl:with-param name="string" select="normalize-space($tmp2)" />
						<xsl:with-param name="type" select="'x'" />
					</xsl:call-template>
				</xsl:when>
				<xsl:otherwise>
					<subfield code="a"><xsl:value-of select="$string" />.</subfield>
				</xsl:otherwise>
			</xsl:choose>
		</datafield>
	</xsl:template>
...
false, 
						...
						<xsl:if test="contains($bool_found,'true')!=0">
							<xsl:for-each select="/ead/archdesc/controlaccess/controlaccess/geogname[@encodinganalog='651']">
								<xsl:if test="contains(normalize-space(.), translate(substring-before($str2, '--'),'.',' '))!=0">
									...
									<xsl:call-template name="subject_tokenize">
										<xsl:with-param name="string" select="normalize-space(substring-after($str2, '--'))" />
										<xsl:with-param name="type" select="'x'" />
									</xsl:call-template>
								</xsl:if>
							</xsl:for-each>
						...
						<xsl:if test="contains($bool_found,'true')=0">
							...
							<xsl:call-template name="subject_tokenize">
								<xsl:with-param name="string" select="normalize-space($newstr2)" />
								<xsl:with-param name="type" select="'x'" />
							</xsl:call-template>
						</xsl:if>
					</xsl:when>
